...
namespace HH.ZK.UI
{
    public partial class Frm学生信息导入 : Form
    {
        ...
        private string GetCellValue(DataGridViewRow row, string colName, bool toupper = false)
        {
            if (row.Cells[colName].Value != null)
            {
                var ret = row.Cells[colName].Value.ToString().Trim();
                if (!string.IsNullOrEmpty(ret))
                {
                    ret = LJH.GeneralLibrary.StringHelper.ToDBC(ret);
        ...
        {
            string msg = null;
            string id = row.Cells["colID"].Value != null ? row.Cells["colID"].Value.ToString().Trim().ToUpper() : null;
