...
{
    public partial class Frm中山中考数据接口 : Form
    {
        public Frm中山中考数据接口()
        {
            InitializeComponent();
        ...
        private 中山中考接口Client _DBClient = null;
        ...
        private void UploadStudentsTask()
        {
            try
            {
                ...
                if (ret != null && ret.body != null && ret.body.Length > 0)
                {
                    ...
                    {
                        提取中山学生信息(ret.body[i], students, gps, photos);
                    }
                ...
                if (students != null && students.Count > 0)
                {
                    ...
                    foreach (var s in students)
                    {
                        ...
                        foreach (var score in s.TestScores)
                        {
                            ...
                            var ret = _DBClient.上传成绩(score, s.获取成绩序号(score));
        ...
        private void UploadStudentCheckInfoTask()
        {
            try
            {
                var con = new StudentSearchCondition() { Checked = true, StudentUploaded = false, PageIndex = 1, PageSize = 50 };
                var students = new APIClient(AppSettings.Current.ConnStr).GetList<string, Student>(con, AppSettings.Current.PhysicalProject.ID).QueryObjects;
                if (students != null && students.Count > 0)
                {
                    int count = 0;
                    ShowProgress(0, p上传检录信息);
                    var patches = new List<UpdateItem<string>>();
                    foreach (var s in students)
                    {
                        count++;
                        var ret = _DBClient.考生检录(s.ID, s.Name);
                        if (ret.Result == ResultCode.Successful) //0成功
                        {
                            patches.Add(new UpdateItem<string>() { ID = s.ID, Key = "UploadTime", Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") });
                            patches.Add(new UpdateItem<string>() { ID = s.ID, Key = "UploadMsg", Value = null });
                        }
                        else
                        {
                            patches.Add(new UpdateItem<string>() { ID = s.ID, Key = "UploadTime", Value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") });
                            patches.Add(new UpdateItem<string>() { ID = s.ID, Key = "UploadMsg", Value = ret.Message });
                        }
                        ShowProgress((decimal)count / students.Count, p上传检录信息);
                    }
                    if (patches.Count > 0) new APIClient(AppSettings.Current.ConnStr).BatchPatch<string, Student>(patches, AppSettings.Current.PhysicalProject.ID);
                }
            }
            catch (ThreadAbortException)
            {
            ...
            this.Invoke((Action)(() => { btn上传检录信息.Text = "开始上传"; }));
        }

        private void 提取中山学生信息(中山中考学生信息 stu, List<Student> students, List<StudentGroup> groups, List<StudentPhoto> photos)
        {
            try
            {
                ...
                if (s == null)
                {
                    ...
                    if (stu.报考科目 != null && stu.报考科目.Length > 0)
                    {
                        foreach (var xm in stu.报考科目)
                        {
                            ...
                            if (string.IsNullOrEmpty(xm.组别)) continue;
                            var pi = AppSettings.Current.PhysicalProject.PhysicalItems?.GetPhysicalItem(xm.测试科目);
                            var sg = groups.SingleOrDefault(it => it.Sex == s.Sex && it.Number == int.Parse(xm.组别) && it.PhysicalItems == xm.测试科目.ToString() && it.FacilityName == s.FacilityID && it.PlanDate == xm.考试时间);
                            if (sg == null)
                            {
                                sg = new StudentGroup()
                                {
                                    ID = Guid.NewGuid(),
                                    Sex = s.Sex,
                                    Number = int.Parse(xm.组别),
                                    PhysicalItems = xm.测试科目.ToString(),
                            ...
                            if (!sg.Items.Exists(it => it.StudentID == s.ID))
                            {
                                sg.Add(new StudentGroupPair()
                                {
                                    ...
                                    GroupID = sg.ID,
                                    StudentID = s.ID,
                                    Channel = !string.IsNullOrEmpty(xm.道次) ? int.Parse(xm.道次) : (int?)null,
                                });
                            }
                        }
                    }
        ...
        {
            if (_ThirdServer == null || string.IsNullOrEmpty(_ThirdServer.Server) || string.IsNullOrEmpty(_ThirdServer.User) || string.IsNullOrEmpty(_ThirdServer.Password))
            {
            ...
            {
                _DBClient = new 中山中考接口Client(_ThirdServer);
                _DBClient.LogIn();
        ...
        {
            if (string.IsNullOrEmpty(txtUrl.Text) || string.IsNullOrEmpty(txtUserID.Text) || string.IsNullOrEmpty(txtPasswd.Text))
            {
                MessageBox.Show("请先设置中山数据上传参数");
                return;
            }
            _ThirdServer = new ThirdPlateFormSettings() { ThirdPlateForm = ThirdPlateForm.中山中考平台 };
            _ThirdServer.Server = txtUrl.Text;
            _ThirdServer.User = txtUserID.Text;
            _ThirdServer.Password = txtPasswd.Text;

            var client = new 中山中考接口Client(_ThirdServer);
            client.LogIn();
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (_UploadStudentCheckInfos == null) btn上传检录信息.PerformClick();
        }
