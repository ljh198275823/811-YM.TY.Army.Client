...
namespace HH.ZK.UI
{
    public partial class Frm学生成绩管理 : FrmReportBaseWithPaging<string, StudentWithTotal>
    {
        ...
        private void mnu删除所选学生_Click(object sender, EventArgs e)
        {
            ...
            DeleteStudents(items);
        }

        private void mnu设置学生类别_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowItemInGridViewRow(dataGridView1.Rows[cell.RowIndex], dataGridView1.Rows[cell.RowIndex].Tag as StudentWithTotal);
            }
        }

        private void mnu设置学生考试科目_Click(object sender, EventArgs e)
        {
            var students = GetSelectedStudents();
            if (students.Count == 0) return;
        ...
        private void mnu取消检录_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("是否取消所选学生的检录状态?", "询问", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
            var sids = items.Select(it => it.ID).ToList();
            if (sids.Count > 0)
            {
                var ret = new APIClient(AppSettings.Current.ConnStr).UndoCheck(sids, AppSettings.Current.PhysicalProject.ID);
