...
namespace HH.ZK.UI
{
    public partial class Frm导入平时成绩 : Form
    {
        public Frm导入平时成绩()
        {
            InitializeComponent();
        ...
        private bool CheckMapping()
        {
            ...
            if (string.IsNullOrEmpty(cmb毕业学校.Text))
            {
                MessageBox.Show("平时成绩没有映射");
                cmb毕业学校.Focus();
                return false;
            }
            return true;
            ...
            try
            {
                ...
                if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (_SourceTable != null)
                    {
                        ...
                        FillColumn(_SourceTable, cmb毕业学校);
                        PreFillDes(cmb毕业学校, "平时成绩");
                        PreFillDes(cmb毕业学校, "加分");
                        PreFillDes(cmb毕业学校, "国标分");
                    }
            ...
            Action action = delegate ()
            {
                try
                {
                    ...
                    for (int i = 0; i < viewDestination.Rows.Count; i++)
                    {
                        ...
                        {
                            string sid = viewDestination.Rows[i].Cells["colID"].Value != null ? viewDestination.Rows[i].Cells["colID"].Value.ToString().Trim() : null;
                            string 平时成绩 = viewDestination.Rows[i].Cells["col平时成绩"].Value != null ? viewDestination.Rows[i].Cells["col平时成绩"].Value.ToString() : null;
                            decimal jiafen;
                            if (!string.IsNullOrEmpty(sid) && !temp.Exists(it => it.ID == sid))
                            {
                                if (decimal.TryParse(平时成绩, out jiafen) && jiafen >= 0)
                                {
                                    temp.Add(new UpdateItem<string>() { ID = sid, Key = "JiaFen", Value = 平时成绩 });
                                    rows.Add(sid, viewDestination.Rows[i]);
                                }
                                else
                                {
                                    viewDestination.Rows[i].Cells["colReason"].Value = "平时成绩不能转化成数字";
                                }
                            }
                        }
                        if (temp.Count >= perTime || i == viewDestination.Rows.Count - 1)
                        {
                            var ret = new WebAPIClient.APIClient(AppSettings.Current.ConnStr).BatchPatch<string, Student>(temp, AppSettings.Current.PhysicalProject.ID);
