...
{
    public partial class FrmCardIDImport : Form
    {
        public FrmCardIDImport()
        {
            InitializeComponent();
        }
        ...
        private bool CheckMapping()
        {
            ...
            if (string.IsNullOrEmpty(cmbCardID.Text))
            {
                MessageBox.Show("物理卡号没有映射");
                cmbCardID.Focus();
                return false;
            }
            return true;
            ...
            Action action = delegate ()
            {
                try
                {
                    ...
                    for (int i = 0; i < viewDestination.Rows.Count; i++)
                    {
                        ...
                        {
                            string sid = viewDestination.Rows[i].Cells["colID"].Value != null ? viewDestination.Rows[i].Cells["colID"].Value.ToString() : null;
                            string cid = viewDestination.Rows[i].Cells["colCardID"].Value != null ? viewDestination.Rows[i].Cells["colCardID"].Value.ToString() : null;
                            if (!string.IsNullOrEmpty(sid) && !temp.Exists(it => it.ID == sid))
                            {
                                temp.Add(new UpdateItem<string>() { ID = sid, Key = "CardID", Value = cid });
