...
{
    public partial class FrmScoreCollect_澳亚特 : Form
    {
        public FrmScoreCollect_澳亚特()
        {
            InitializeComponent();
        }
        ...
        private 澳亚特ScoreReader _Reader = null;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            var temp = AppSettings.Current.GetConfigContent("AoYeTeIP");
            if (!string.IsNullOrEmpty(temp)) txtIP.IP = temp;
            temp = AppSettings.Current.GetConfigContent("AoYeTePort");
            if (!string.IsNullOrEmpty(temp)) txtPort.Text = temp;
        }

        private void btnBegin_Click(object sender, EventArgs e)
        {
            if (txtIP.IP != "0.0.0.0" && txtPort.IntergerValue > 0)
            {
                _Reader = new 澳亚特ScoreReader(txtIP.IP, txtPort.IntergerValue);
                _Reader.IsRealTime = rdRealTime.Checked;
                if (_Reader.IsRealTime)
                {
                    var temp = AppSettings.Current.GetConfigContent("AoYeTeLastSN");
                    int lastSN = 0;
                    if (!string.IsNullOrEmpty(temp) && int.TryParse(temp, out lastSN)) _Reader.LastSN = lastSN;
                }
                ...
                timer1.Enabled = true;
                AppSettings.Current.SaveConfig("AoYeTeIP", txtIP.IP);
                AppSettings.Current.SaveConfig("AoYeTePort", txtPort.Text);
            }
        }
        ...
        private void _Reader_ReadCompleted(object sender, EventArgs e)
        {
            Action action = delegate ()
            {
                ...
                MessageBox.Show("数据采集完成", "消息", MessageBoxButtons.OK, MessageBoxIcon.Information);
            };
            this.Invoke(action);
        }
